...
const _sections = <_Section>[
  _Section('issue_for', 'Issue For', Icons.groups_rounded,
      '"Issue For" dropdown — which department the issue is raised against.'),
  _Section('issue_types', 'Issue Type', Icons.build_circle_rounded,
      '"Issue Type" dropdown in the Report an Issue form.'),
];

class EditMaintenanceFormPage extends StatefulWidget {
  const EditMaintenanceFormPage({super.key});

  @override
  State<EditMaintenanceFormPage> createState() => _EditMaintenanceFormPageState();
}

class _EditMaintenanceFormPageState extends State<EditMaintenanceFormPage> {
  bool _loading = true;
  bool _saving  = false;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      _config = {
        'issue_for':   MaintenanceFormConfig.getIssueForOptions(cfg),
        'issue_types': MaintenanceFormConfig.getIssueTypes(cfg),
      };
      _nextVersionNumber = await SupabaseService.getNextMaintenanceFormVersionNumber();
    } catch (_) {
      final def = MaintenanceFormConfig.defaults();
      _config = {
        'issue_for':   MaintenanceFormConfig.getIssueForOptions(def),
        'issue_types': MaintenanceFormConfig.getIssueTypes(def),
      };
    }
  ...
  Future<void> _loadHistory() async {
    ...
    final versions = await SupabaseService.fetchMaintenanceFormVersions();
  ...
  Future<void> _publishDirectly() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'This will immediately update the Maintenance issue-report form for everyone.',
    ...
    try {
      await SupabaseService.saveMaintenanceFormVersion({
        'form_config': _config,
        'status': 'approved',
      ...
      if (mounted) {
        ...
        _snack('Maintenance form options published successfully.', _green);
        await _loadHistory();
      }
    } catch (e) {
  ...
  Future<void> _approveVersion(Map<String, dynamic> version) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'This will approve and immediately apply these form changes for everyone.',
            style: TextStyle(fontSize: 13, color: Color(0xFF6B7280))),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')),
          ElevatedButton(
  ...
  Future<void> _editOptions(String key, String label) async {
                    ...
                    child: ConstrainedBox(
                      ...
                      child: Column(
                        ...
                        children: [
                              ...
                              Expanded(
                                child: Text(
                                  _isManagement
                                      ...
                                      : 'Edit dropdown options and tap "Send for Approval". Management will review your changes before they go live.',
                          ...
                          const SizedBox(height: 12),
                          _FormGroupCard(config: _config, onEdit: _editOptions),
...
class _FormGroupCard extends StatelessWidget {
  ...
  const _FormGroupCard({required this.config, required this.onEdit});
        ...
        Container(
          ...
          child: Row(children: [
            Container(
              ...
              child: Icon(Icons.build_rounded, color: c, size: 17),
            ),
            const SizedBox(width: 10),
            Text('Report an Issue Form',
